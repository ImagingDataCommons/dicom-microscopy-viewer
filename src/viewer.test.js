...
describe('test detectDisplayColorSpace function', () => {
  ...
  it('should detect display-p3 color space when supported', () => {
    ...
    const viewer = new dmv.viewer.VolumeImageViewer({
      client: {},
      metadata: testCase1.images.map(metadata => {
        return new dmv.metadata.VLWholeSlideMicroscopyImage({ metadata })
      }),
    })

    expect(viewer.getICCOutputType()).toBeDefined()
    expect(viewer.getICCOutputType()).toEqual('display-p3')
  })
  ...
  it('should fall back to srgb when p3 is not supported', () => {
    ...
    expect(viewer.getICCOutputType()).toBeDefined()
    expect(viewer.getICCOutputType()).toEqual('srgb')
  })
