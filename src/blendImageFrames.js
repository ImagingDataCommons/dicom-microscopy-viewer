...
function initShaders () {
  Object.keys(fragmentShaders).forEach(id => {
    // Console.log("WEBGL: Loading shader", id);
    const frag = fragmentShaders[id];

    shader.attributes = {};
    shader.uniforms = {};

    // todo: use fragmentShaders.int8, ...
    shader.program = createProgramFromString(gl, vertexShader, frag);
 ...
 */
function getImageDataType (pixelData, color = false) {
  if (color) {
    return 'rgb';
...
function getShaderProgram (pixelData, color) {

  const datatype = getImageDataType(pixelData, color);
...
function generateTexture (pixelData, width, height, color = false) {
  ...
  const imageDataType = getImageDataType(pixelData, color);
...
function render (pixelData, width, height, color, windowCenter, windowWidth, colorLUT) {
  // Resize the canvas
  ...
  const shader = getShaderProgram(pixelData, color);
  const texture = generateTexture(pixelData, width, height, color);
 ...
 */
function blendImageFrames(frameData) {
  const canvas = document.createElement('canvas');
  const context = canvas.getCanvasContext('2d');
  // Step 1:
  // Verify all frames have the same dimensions, or we can't blend them?

  // maybe later...
  // Split Frames into batches of maxTextures / 2
  // const maxTextures = gl.getParameter(gl.MAX_TEXTURE_IMAGE_UNITS);

  // TODO: add texture for colorLUT


  // For visible frames, batch into groups below max texture size
  const renderedCanvas = render(pixelData, width, height, color, windowWidth, windowCenter, colorLUT);

  context.drawImage(renderedCanvas, 0, 0);

  return canvas; // or grab pixel data ? maybe need .toImageSrc()
}
