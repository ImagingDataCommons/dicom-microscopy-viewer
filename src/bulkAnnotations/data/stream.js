...
} from './prefixEmitter.js'

function debugLog(message, details) {
  console.debug(`[bulkAnnotations] ${message}`, details)
}

/** @typedef {'int32' | 'float32'} StreamableBulkKind */

/** @typedef {Int32Array | Float32Array} StreamableBulkGraphicArray */

/**
 * @typedef {object} StreamableVrInfo
 * @property {number} elementByteSize
 * @property {StreamableBulkKind} kind
 */
 ...
 *
 * @param {string} vr
 * @returns {StreamableVrInfo|null}
 */
export function getStreamableBulkVrInfo(vr) {
  switch (vr) {
    case 'OL':
...
/**
 * Attribute-name based typing: prefer inferring streamability from the DICOM
 * keyword rather than the (sometimes missing or unreliable) `vr` in DICOM
 * JSON bulkdata stubs. `PointCoordinatesData` (0066,0016) is always OF
 * (32-bit float) and `LongPrimitivePointIndexList` (0066,0040) is always OL
 * (32-bit int); `DoublePointCoordinatesData` (0066,0022) is OD (64-bit) and is
 * not streamable here.
 *
 * @param {string} attributeName - DICOM keyword of the bulkdata attribute
 * @returns {StreamableVrInfo|null}
 */
export function getStreamableVrInfoForAttribute(attributeName) {
  switch (attributeName) {
    case 'PointCoordinatesData':
      return getStreamableBulkVrInfo('OF')
    case 'LongPrimitivePointIndexList':
      return getStreamableBulkVrInfo('OL')
    default:
      return null
  }
}
...
/**
 * @param {StreamableBulkKind} kind
 * @param {ArrayBuffer} buffer
 * @param {number} elementCount
 * @returns {StreamableBulkGraphicArray}
 */
function makeGraphicDataView(kind, buffer, elementCount) {
  return kind === 'int32'
...
/**
 * @typedef {object} BulkDataReference
 * @property {string} [vr]
 ...
 * @param {object} options.metadataItem
 * @param {object} [options.bulkdataItem]
 * @returns {BulkDataReference|null}
 */
export function resolveStreamableGraphicDataReference(options) {
  const { metadataItem, bulkdataItem } = options
  ...
  const pointRef = bulkdataItem.PointCoordinatesData
  const doubleRef = bulkdataItem.DoublePointCoordinatesData
 ...
 *
 * @param {Int32Array} graphicIndex
 * @param {number} numberOfAnnotations
 * @returns {boolean}
 */
export function isMonotonicGraphicIndex(graphicIndex, numberOfAnnotations) {
...
/**
 * Validate a `graphicIndex` (`LongPrimitivePointIndexList`) against the basic
 * invariants prefix-decoding relies on: it must start at 1 (one-based
 * offset), be monotonically non-decreasing, and every entry must land on a
 * coordinate-tuple boundary (`(index - 1) % coordinateDimensionality === 0`).
 *
 * @param {Int32Array} graphicIndex
 * @param {number} numberOfAnnotations
 * @param {number} coordinateDimensionality - 2 for (x, y), 3 for (x, y, z)
 * @returns {{ok: boolean, errors: string[]}}
 */
export function validateGraphicIndex(
  graphicIndex,
  numberOfAnnotations,
  ...
  const n = Math.min(numberOfAnnotations, graphicIndex.length)
  const dim =
    Number.isFinite(coordinateDimensionality) && coordinateDimensionality > 0
      ? coordinateDimensionality
      : 1
  ...
  if (Number(graphicIndex[0]) !== 1) {
    errors.push(
      `graphicIndex[0] must be 1 (one-based offset), got ${graphicIndex[0]}`,
    )
  }

  if (!isMonotonicGraphicIndex(graphicIndex, numberOfAnnotations)) {
    errors.push('graphicIndex is not monotonically non-decreasing')
  }

  for (let i = 0; i < n; i++) {
    const value = Number(graphicIndex[i])
    if ((value - 1) % dim !== 0) {
      errors.push(
        `graphicIndex[${i}] = ${value} is not aligned to coordinate dimensionality ${dim}`,
      )
    }
  }
...
export function browserSupportsBulkStreaming() {
  return (
...
function indexOfSubarray(haystack, hayLen, needle, fromIndex) {
...
function lastIndexOfSubarray(haystack, hayLen, needle) {
  const needleLen = needle.length
 ...
 *
 * @callback BulkDataRangeRetriever
 * @param {object} options
 * @param {string} options.BulkDataURI
 * @param {Array<{mediaType: string, transferSyntaxUID?: string}>} [options.mediaTypes]
 * @param {[number, number]} [options.byteRange]
 * @returns {Promise<ArrayBuffer[]>}
 */

/**
 * @typedef {object} StreamBulkGraphicDataOptions
 * @property {string} url - Absolute or relative `BulkDataURI`
 * @property {string} [baseUrl] - Base URL used to resolve a relative `BulkDataURI`
 * @property {Record<string, string>} headers - Auth + other headers (e.g. `Authorization: Bearer …`)
 * @property {string} vr
 * @property {Int32Array} graphicIndex
 * @property {number} numberOfAnnotations
 * @property {AbortSignal} [signal]
 * @property {BulkDataRangeRetriever} [retrieveBulkData] - Preferred progressive path: sequential `byteRange` retrieves through dicomweb-client XHR. This is what actually paints mid-transfer against IDC/GCP proxies that buffer a full multipart GET.
 * @property {RequestCredentials} [credentials] - Fetch credentials mode. Default `omit` matches dicomweb-client XHR (`withCredentials` false). Do not use `include` against IDC/GCP proxies — it triggers a CORS preflight that fails unless the server sends `Access-Control-Allow-Credentials: true`.
 * @property {(loadedBytes: number, totalBytes: number|null) => void} [onProgress] - Network progress (whole-response bytes)
 * @property {(info: BulkPrefixInfo) => void|Promise<void>} [onPrefix] - Newly-available, element-aligned coordinate prefix (throttled)
 * @property {number} [prefixThrottleBytes] - Emit a prefix at most this often by payload bytes (default 128 KiB)
 * @property {number} [prefixEmitAnnotationStep] - Also emit when this many additional annotations become fully decodable (default 2_000). Keeps paint moving when the browser delivers one large read at the end of a fast download.
 * @property {number} [rangeChunkBytes] - HTTP Range chunk size for progressive retrieve (default 1 MiB). Smaller → more frequent paints; larger → fewer round-trips.
 * @property {boolean} [preferByteRange] - Use raw `fetch` + Range (separate from `retrieveBulkData`). Default false — enable with `localStorage.slim:vivBulkRange=1` only for debugging.
 */

/** Own a tightly-sized buffer so TypedArray views cover only payload bytes. */
function ownedPayloadBytes(src) {
 ...
 *
 * @param {StreamBulkGraphicDataOptions} options
 * @returns {Promise<StreamableBulkGraphicArray>}
 */
export async function streamBulkGraphicData(options) {
  const {
    url,
    baseUrl,
    headers,
    ...
    try {
      ...
      const status =
        e != null && typeof e === 'object' && 'status' in e
          ? Number(e.status)
          : Number.NaN
      debugLog('bulkStream:client Range failed; trying full GET', {
        status: Number.isFinite(status) ? status : undefined,
        err: e instanceof Error ? e.message : String(e),
      })
      console.warn(
        status === 206
          ? '[bulkAnnotations] dicomweb-client rejected HTTP 206 Partial Content; falling back to full GET (paint after download).'
          : '[bulkAnnotations] Range progressive retrieve failed; falling back to full GET (paint after download).',
        e,
      )
    }
  }
  ...
  if (preferFetchRange) {
    try {
      ...
      debugLog('bulkStream:fetch Range probe failed; using full GET', {
        err: e instanceof Error ? e.message : String(e),
      })
    }
  }
...
/**
 * @param {ArrayBuffer|Uint8Array} part
 * @returns {Uint8Array}
 */
function partToUint8(part) {
 ...
 */
function isRangeEndOfStreamError(e) {
  if (e == null) {
    return false
  }
  if (typeof e === 'object' && 'status' in e && Number(e.status) === 416) {
    return true
  }
 ...
 *
 * @param {object} options
 * @returns {Promise<StreamableBulkGraphicArray|null>}
 */
async function streamBulkGraphicDataViaClientRanges(options) {
  ...
  if (firstChunk.byteLength > firstRequestedBytes) {
    debugLog('bulkStream:client Range ignored (oversized first part)', {
      firstBytes: firstChunk.byteLength,
      rangeChunkBytes,
    })
    console.warn(
      '[bulkAnnotations] server ignored HTTP Range (got full body in one response). Mid-transfer paint needs proxy Range support; painting all annotations once from the buffered payload instead of re-downloading.',
  ...
  while (moreRemaining) {
    ...
    try {
      ...
      /**
       * Re-read via helper: AbortSignal.aborted can flip mid-await, and a
       * pre-await check could be stale by the time we get here.
       */
      ...
      if (rangeIndex > 0 && isRangeEndOfStreamError(e)) {
        debugLog('bulkStream:client Range EOF (past-end request)', {
          start,
          end,
          err: e instanceof Error ? e.message : String(e),
        })
    ...
    if (chunk.byteLength > requestedBytes) {
      console.warn(
        '[bulkAnnotations] server ignored HTTP Range mid-stream (response exceeds requested slice); completing from the returned full body instead of appending.',
 ...
 * @returns {Promise<StreamableBulkGraphicArray|null>}
 */
async function tryStreamBulkGraphicDataViaRanges(options) {
  const {
    resolvedUrl,
    headers,
    credentials,
  ...
  if (firstResponse.status === 200) {
     ...
     */
    debugLog('bulkStream:range ignored (HTTP 200); reading body', {
      url: resolvedUrl,
    })
  ...
  if (firstResponse.status !== 206) {
    debugLog('bulkStream:range unsupported; falling back to full GET', {
  ...
  if (totalBytes == null || !Number.isFinite(totalBytes) || totalBytes <= 0) {
    debugLog('bulkStream:range missing Content-Range total; fallback', {
  ...
  const readResponseIntoPayload = async (response) => {
  ...
  while (payloadLen < totalBytes) {
    ...
    if (response.status === 200) {
      ...
      fullBodyRestarts += 1
      console.warn(
        '[bulkAnnotations] server ignored Range mid-stream (HTTP 200); restarting buffer with the full response body.',
        { start, end },
      )
      payloadLen = 0
 ...
 * @returns {Promise<StreamableBulkGraphicArray>}
 */
async function streamBulkGraphicDataFullGet(options) {
  const {
    resolvedUrl,
    headers,
    credentials,
...
async function consumeBulkBodyStream(options) {
  ...
  if (likelyBufferedByProxy) {
    ...
    console.warn(
      '[bulkAnnotations] response looks buffered by origin/proxy (few huge reads after long wait). Prefer Range path; check Network for 206 Partial Content.',
